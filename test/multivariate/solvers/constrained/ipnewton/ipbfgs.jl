@testset "Interior point BFGS" begin
    # TODO: There is an equivalent setstate! function in constraints.jl
    ...
    @testset "IPBFGS computations" begin
        ...
        μ = 0.2345678
        d0 = OnceDifferentiable(x->0.0, (g,x)->fill!(g, 0.0), rand(3))
        A = randn(3,3); H = A'*A
        dg = OnceDifferentiable(x->(x'*H*x)[1]/2, (g,x)->(g[:] = H*x), rand(3))
        x = clamp.(randn(3), -0.99, 0.99)
        gx = similar(x)
        cfun = x->Float64[]
        c = Float64[]
        J = Array{Float64}(0,0)
        method = Optim.IPBFGS(μ0 = μ)
        ...
        @test f_x == L == dg.f(x)
        @test gx == H*x
        constraints = OnceDifferentiableConstraints(
            (c,x)->nothing, (J,x)->nothing, bounds)
        ...
        bounds = Optim.ConstraintBounds([], [], xbar, xbar)
        constraints = OnceDifferentiableConstraints(cvar!, cvarJ!, bounds)
        state = Optim.initial_state(method, options, d0, constraints, x)
        copy!(state.bstate.λcE, bstate.λxE)
        setstate!(state, μ, d0, constraints, method)
        @test Optim.gf(bounds, state) ≈ [gx; xbar-x]
        # TODO: Test invHf instead of Hf for BFGS
        #n = length(x)
        #@test Optim.Hf(constraints, state) ≈ [eye(n,n) -eye(n,n); -eye(n,n) zeros(n,n)]
        ...
        # Now again using the generic machinery
        bounds = Optim.ConstraintBounds([], [], zeros(length(x)), fill(Inf,length(x)))
        constraints = OnceDifferentiableConstraints(cvar!, cvarJ!, cvarh!, bounds)
        state = Optim.initial_state(method, options, d0, constraints, y)
        setstate!(state, μ, d0, constraints, method)
        @test Optim.gf(bounds, state) ≈ -μ./y
        # TODO: Test invHf instead of Hf for BFGS
        ...
        # TODO: Fix autodiff check
        #check_autodiff(d0, bounds, x, cfun, bstate, μ)
        constraints = OnceDifferentiableConstraints(cfun!, cJ!, bounds)
        state = Optim.initial_state(method, options, d0, constraints, x)
    ...
    @testset "IPBFGS initialization" begin
        method = IPBFGS()
        options = Optim.Options(; Optim.default_options(method)...)
        x = [1.0,0.1,0.3,0.4]
        ## A linear objective function (hessian is zero)
        f_g = [1.0,2.0,3.0,4.0]
        d = OnceDifferentiable(x->dot(x, f_g), (g,x)->copy!(g, f_g), x)
        # Variable bounds
        constraints = OnceDifferentiableConstraints([0.5, 0.0, -Inf, -Inf], [Inf, Inf, 1.0, 0.8])
        ...
        d = OnceDifferentiable(x->sum(hd.*x.^2)/2, (g,x)->copy!(g, hd.*x),  x)
    ...
    @testset "IPBFGS step" begin
        function autoqp(d, constraints, state)
            ...
            Tx = typeof(state.x)
            stated = convert(Optim.IPBFGSState{TD, Tx,1}, state)
            # TODO: Uncomment
            #stated2 = convert(Optim.IPBFGSState{TD2, Tx, 1}, state)
        ...
        F = 1000
        d = OnceDifferentiable(x->F*x[1], (g, x) -> (g[1] = F), [0.0,])
        μ = 1e-20
        method = Optim.IPBFGS(μ0=μ)
        options = Optim.Options(; Optim.default_options(method)...)
        x0 = μ/F*10  # minimum is at μ/F
        # Nonnegativity (the case that doesn't require slack variables)
        constraints = OnceDifferentiableConstraints([0.0], [])
        ...
        # TODO: Add linesearch_anon tests for IPBFGS(linesearch! = backtracking_constrained)
    end
    ...
    @testset "Slack" begin
        ...
        for σ in (1, -1)
            ...
            d = OnceDifferentiable(x->F*x[1], (g, x) -> (g[1] = F), [0.0,])
            constraints = OnceDifferentiableConstraints(σswap(σ, [0.0], [])...)
            ...
            d = OnceDifferentiable(x->F*(x[1]-σ), (g, x) -> (g[1] = F), [0.0,])
            constraints = OnceDifferentiableConstraints(σswap(σ, [Float64(σ)], [])...)
